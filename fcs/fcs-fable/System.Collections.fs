...
module Immutable =
    ...
    // not actually immutable, just an extension of ResizeArray
    type ImmutableArray<'T>() =
        inherit ResizeArray<'T>()
        static member CreateBuilder() = ImmutableArray<'T>()

    // not actually immutable, just an extension of Dictionary
